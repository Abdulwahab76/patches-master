...
const AIAssistant: React.FC = () => {
  ...
  return (
    <section
      id="ai-assistant"
      className="py-24 bg-orange-50 border-y border-orange-100"
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="bg-orange-200 text-orange-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest">
            AI Innovation
          </span>
          ...
          <p className="mt-4 text-slate-600">
            Our AI Design Assistant can suggest concepts for your brand in
            seconds.
          </p>
        </div>
          ...
          {ideas.length > 0 && (
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {ideas.map((idea, i) => (
                <div
                  key={i}
                  className="bg-slate-50 p-6 rounded-2xl border border-slate-100 hover:border-orange-300 transition group"
                >
                  <h4 className="font-bold text-lg mb-2 text-slate-900">
                    {idea.title}
                  </h4>
                  <p className="text-sm text-slate-600 mb-4 line-clamp-3 group-hover:line-clamp-none transition-all">
                    {idea.concept}
                  </p>
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (target) {
                        ...
                        ).value = `Based on AI Idea: ${idea.title}\nConcept: ${idea.concept}\nStyle: ${idea.style}`;
